...
def prt_skim_setup(period, mode, vot):
    ...
    PER_TTC     = h.GetMulti(Visum.Net.Links, period+r"_TTC"    , activeOnly = True)
    ...
    # Pull Toll Fields
    PER_SOV_TOLL = h.GetMulti(Visum.Net.Links,period+r"_SOV_TOLL", activeOnly = True)
    PER_SR2_TOLL = h.GetMulti(Visum.Net.Links,period+r"_SR2_TOLL", activeOnly = True)
    PER_SR3_TOLL = h.GetMulti(Visum.Net.Links,period+r"_SR3_TOLL", activeOnly = True)
    PER_MT_TOLL  = h.GetMulti(Visum.Net.Links,period+r"_MT_TOLL" , activeOnly = True)
    PER_HT_TOLL  = h.GetMulti(Visum.Net.Links,period+r"_HT_TOLL" , activeOnly = True)
    ...
    att_list = [PER_TTC,PER_SOV_TOLL,PER_SR2_TOLL,PER_SR3_TOLL,PER_MT_TOLL,PER_HT_TOLL]
    ...
    df = pd.DataFrame(np.column_stack(att_list), columns = ['PER_TTC','PER_SOV_TOLL','PER_SR2_TOLL','PER_SR3_TOLL','PER_MT_TOLL','PER_HT_TOLL'])
                #'EA_TTC','AM_TTC','MD_TTC','PM_TTC','EV_TTC',
                #'EA_SOV_TOLL','EA_SR2_TOLL','EA_SR3_TOLL','EA_MT_TOLL','EA_HT_TOLL',
                #'AM_SOV_TOLL','AM_SR2_TOLL','AM_SR3_TOLL','AM_MT_TOLL','AM_HT_TOLL',
                #'MD_SOV_TOLL','MD_SR2_TOLL','MD_SR3_TOLL','MD_MT_TOLL','MD_HT_TOLL',
    ...
    if   mode == "SOV" and vot == "Low":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sov_low) * 3600               # Gen Cost (Time in Seconds)
    # SR2, Low VOT
    elif mode == "SR2" and vot == "Low":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr2_low) * 3600               # Gen Cost (Time in Seconds)
    # SR3, Low VOT
    elif mode == "SR3" and vot == "Low":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr3_low) * 3600               # Gen Cost (Time in Seconds)
    # SOV, Medium VOT
    elif mode == "SOV" and vot == "Medium":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sov_med) * 3600               # Gen Cost (Time in Seconds)
    # SR2, Medium VOT
    elif mode == "SR2" and vot == "Medium":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr2_med) * 3600               # Gen Cost (Time in Seconds)
    # SR3, Medium VOT
    elif mode == "SR3" and vot == "Medium":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr3_med) * 3600               # Gen Cost (Time in Seconds)
    # SOV, High VOT
    elif mode == "SOV" and vot == "High":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sov_high) * 3600              # Gen Cost (Time in Seconds)
    # SR2, High VOT
    elif mode == "SR2" and vot == "High":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr2_high) * 3600              # Gen Cost (Time in Seconds)
    # SR3, High VOT
    elif mode == "SR3" and vot == "High":
        df['GenCost'] = df['PER_TTC'] + (df['PER_'+mode+'_TOLL'] / sr3_high) * 3600              # Gen Cost (Time in Seconds)
                                           

    # Set fields back in Visum
    # Period Travel Time
    h.SetMulti(Visum.Net.Links ,r"ADDVAL1", df['PER_TTC'], activeOnly = True)   # Time in seconds
    # Gen Cost
    h.SetMulti(Visum.Net.Links ,r"ADDVAL2", df['GenCost'], activeOnly = True)   # Gen Cost (Time in Seconds)
    # Toll
    if mode == "SOV":
        h.SetMulti(Visum.Net.Links ,r"TOLL_PRTSYS(S)",   df['PER_'+mode+'_TOLL'], activeOnly = True)   # Money in Dollars 
    elif mode == "SR2":
        h.SetMulti(Visum.Net.Links ,r"TOLL_PRTSYS(SR2)", df['PER_'+mode+'_TOLL'], activeOnly = True)   # Money in Dollars 
    elif mode == "SR3":
        h.SetMulti(Visum.Net.Links ,r"TOLL_PRTSYS(SR3)", df['PER_'+mode+'_TOLL'], activeOnly = True)   # Money in Dollars 
